...
- `factor_lab/adapters/proposal.py`
  - `ProjectQlibFactorHypothesisGen.prepare_context()` 经 **`ctx["RAG"]`** 注入宪法 + 多样性 + 学术示例 + 负向 AVOID（非 `scenario`）
  - `_enrich_factor_hypothesis_rag()` 对齐 quant 模式 `compose_project_rag`

### 3.4 Phase 4 — 预筛选器

- 新建 `factor_lab/adapters/pre_screener.py`
  - `filter_new_factors_panel()`：merge 前按列 Rank IC 筛选
  - 默认 **关闭**，需 `FACTOR_LAB_PRESCREENER_ENABLED=1`
  - 默认 `MIN_IC=0.015`（2026-05-23）
- `factor_lab/adapters/patch_qlib_conda.py` 包装 `process_factor_data`（真实 qrun 路径）
...
| 6   | 预筛选器接入 qrun 前                    | `factor_lab/adapters/patch_qlib_conda.py` | ✅ `process_factor_data` 包装；默认 MIN_IC=0.015                      |
...
FACTOR_LAB_PRESCREENER_MIN_IC=0.015      # 最低 |Rank IC|（生产默认）
...
| `scripts/lab/production_gate.sh`                            | 生产门禁（benchmark→单测→loop→refresh→train） |
| `scripts/lab/check_qlib_benchmark.py`                       | 中证500 指数 `000905.SZ` 前置检查（`SH000905` 为别名） |
...
| 2026-05-23 | v1.1 | 生产门禁包：PortAnaRecord 恢复、diversity 从 parquet、RAG 注入 ctx['RAG']、负向 loop_patterns、宪法禁用 sw_l1、production_gate.sh |
