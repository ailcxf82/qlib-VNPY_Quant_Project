...
- ✅ **使用标签收益**：收益计算基于 `Ref($close_qfq, -3)/Ref($close_qfq, 1) - 1`（以 T+1 收盘价为基准的 3 日远期相对收益，当前主工程口径，以 `config/data.yaml` 为准），而非实际交易价格
...
其中：
- `weight_i`：股票 i 的权重
- `label_i`：股票 i 的标签收益 = `Ref($close_qfq, -3)/Ref($close_qfq, 1) - 1`（以 T+1 收盘价为基准的 3 日远期相对收益）
